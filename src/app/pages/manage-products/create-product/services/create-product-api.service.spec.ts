...
describe('CreateProductApiService', () => {
  ...
  describe('getAllCategories', () => {
    it('deve retornar as categorias', () => {
      ...
      const req = httpTestingController.expectOne(`${environment.apiUrl}/products/categories`); // faz uma requisição para a api, fazendo o getAllCategories ser disparado
      expect(req.request.method).toBe('GET');
      req.flush(mockCategories); // e retorna os dados mockados
    });
  });
  ...
  describe('getCountProduct', () => {
    it('deve retornar a quantidade de produtos', () => {
      ...
      httpTestingController.verify(); // verifica se a conexão deve ser fechada ou não
    });
  });
});
